...
export interface IStorage {
  ...
  getInspirationNodesByMoodBoard(
    moodBoardId: number
  ): Promise<InspirationNode[]>;
  createInspirationNode(node: InsertInspirationNode): Promise<InspirationNode>;
  updateInspirationNode(
    id: number,
    node: Partial<InspirationNode>
  ): Promise<InspirationNode | undefined>;
  ...
  createNodeConnection(
    connection: InsertNodeConnection
  ...
  getTeamMoodSummary(
    projectId: number
  ): Promise<Record<string, { count: number; intensity: number }>>;
}
...
export class MemStorage implements IStorage {
  ...
  private initSampleData() {
    ...
    const user1: User = {
      ...
      name: "Nithish Raja",
      email: "nithishrajadesign@gmail.com",
      role: "Design Lead",
      avatarUrl: null,
      biography:
        "Experienced design lead with 5+ years in UX/UI design specializing in creating intuitive, scalable, and visually engaging digital solutions across web, mobile, and interactive platforms.",
    ...
    const project1: Project = {
      ...
      description:
        "A complete redesign of the meditation and wellness app focusing on a more intuitive and calming user experience, with innovative sensory feedback.",
      coverImageUrl: "",
    ...
    const project2: Project = {
      ...
      description:
        "Next-generation travel platform with multi-sensory destination previews that engage sight, sound, and emotional connection.",
      coverImageUrl: "",
    ...
    const moodBoard1: MoodBoard = {
      ...
      description:
        "Exploring color psychology, typography, and visual elements for inducing calmness and focus",
    ...
    const moodBoard2: MoodBoard = {
      ...
      description:
        "Gesture-based navigation concepts and fluid transitions between meditation states",
    ...
    const moodBoard3: MoodBoard = {
      ...
      description:
        "Multi-sensory elements capturing the essence of travel destinations through color, sound, and texture",
    ...
    const moodBoard4: MoodBoard = {
      ...
      description:
        "Non-intrusive UI components that adapt to user mood and destination context",
    ...
    const node1: InspirationNode = {
      ...
      description:
        "Soft blues, lavenders, and gentle greens that research shows reduce anxiety and promote mindfulness",
      content: "{}",
      contentUrl:
        "https://images.unsplash.com/photo-1557683311-eac922347aa1?auto=format&fit=crop&w=600&h=400",
      positionX: 100,
      positionY: 150,
      width: 320,
      height: 240,
    ...
    const node2: InspirationNode = {
      ...
      description:
        "The app should function as a 'mood responsive environment' - subtly shifting its appearance and behavior based on the user's detected emotional state",
    ...
    const node4: InspirationNode = {
      ...
      description:
        "Comprehensive set of finger movements that correspond to different meditation actions",
      content: "{}",
      contentUrl:
        "https://images.unsplash.com/photo-1500462918059-b1a0cb512f1d?auto=format&fit=crop&w=400&h=300",
      positionX: 150,
      positionY: 180,
      width: 400,
      height: 300,
    ...
    const node5: InspirationNode = {
      ...
      description:
        "Animation concept for guiding breathing - expanding circle with soft glow effect that signals inhale/exhale timing",
      content: JSON.stringify({
        paths:
          "<svg viewBox='0 0 400 400'><circle cx='200' cy='200' r='100' fill='none' stroke='#7B68EE' stroke-width='3' /><circle cx='200' cy='200' r='150' fill='none' stroke='#7B68EE' stroke-width='2' stroke-opacity='0.6' /><circle cx='200' cy='200' r='180' fill='none' stroke='#7B68EE' stroke-width='1' stroke-opacity='0.3' /></svg>",
      }),
      contentUrl: null, // Added to match schema
      positionX: 350,
      positionY: 220,
    ...
    const node6: InspirationNode = {
      ...
      description:
        "Low-frequency tones that synchronize with breathing exercise and adjust in timbre based on user's stress level",
      content: JSON.stringify({
        duration: 45,
        waveform: "sine",
      }),
    ...
    const node7: InspirationNode = {
      ...
      description:
        "Color extraction from traditional Japanese gardens showing how natural elements can be translated into UI components",
      content: "{}",
      contentUrl:
        "https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=600&h=400",
      positionX: 120,
      positionY: 150,
      width: 400,
      height: 300,
    ...
    const node8: InspirationNode = {
      ...
      description:
        "Audio capture of Las Ramblas with distinctive crowd energy and architectural acoustics",
      content: JSON.stringify({
        duration: 120,
        waveform: "complex",
      }),
    ...
    const node9: InspirationNode = {
      ...
      description:
        "How do we effectively translate the feeling of 'being there' into digital interfaces? Each destination has unique sensory signatures that can be abstracted into UI patterns.",
      content: JSON.stringify({
    ...
    const node10: InspirationNode = {
      ...
      description:
        "UI elements that morph based on destination context - showing how the same functional elements adapt their appearance",
      content: JSON.stringify({
        paths:
          "<svg viewBox='0 0 500 300'><rect x='50' y='50' width='400' height='60' rx='30' fill='rgba(74, 144, 226, 0.2)' stroke='#4A90E2' /><rect x='50' y='130' width='400' height='60' rx='30' fill='rgba(240, 140, 50, 0.2)' stroke='#F08C32' /><rect x='50' y='210' width='400' height='60' rx='5' fill='rgba(46, 204, 113, 0.2)' stroke='#2ECC71' /></svg>",
      }),
      contentUrl: null, // Added to match schema
      positionX: 180,
      positionY: 150,
  ...
  async updateMoodBoard(
    id: number,
    moodBoardUpdate: Partial<MoodBoard>
  ): Promise<MoodBoard | undefined> {
  ...
  async getInspirationNodesByMoodBoard(
    moodBoardId: number
  ): Promise<InspirationNode[]> {
  ...
  async createInspirationNode(
    insertNode: InsertInspirationNode
  ): Promise<InspirationNode> {
  ...
  async updateInspirationNode(
    id: number,
    nodeUpdate: Partial<InspirationNode>
  ): Promise<InspirationNode | undefined> {
  ...
  async getNodeConnections(moodBoardId: number): Promise<NodeConnection[]> {
    const moodBoardNodes = await this.getInspirationNodesByMoodBoard(
      moodBoardId
    );
  ...
  async createNodeConnection(
    insertConnection: InsertNodeConnection
  ): Promise<NodeConnection> {
  ...
  async getTeamMoodSummary(
    projectId: number
  ): Promise<Record<string, { count: number; intensity: number }>> {
    const teamMoods = await this.getTeamMoods(projectId);
    const summary: Record<
      string,
      { count: number; intensity: number; total: number }
    > = {};
