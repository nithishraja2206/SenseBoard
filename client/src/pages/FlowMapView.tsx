        ...
        <div className="flex justify-between items-center">
          <SimpleBreadcrumb
            items={[
              ...
              {
                label: moodBoard?.name || "Mood Board",
            ...
            <Card className="h-[600px] overflow-hidden">
              ...
              <CardContent className="h-full">
                <div className="relative w-full h-full">
                  ...
                  <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-24 h-24 rounded-full primary-gradient flex items-center justify-center z-10">
                  ...
                  <svg className="absolute inset-0 w-full h-full">
                    ...
                    {Object.keys(nodesByMood).map((mood, index) => {
                      const angle =
                        ((2 * Math.PI) / Object.keys(nodesByMood).length) *
                        index;
                      const x1 = 50;
                      const y1 = 50;
                    ...
                    {Object.entries(nodeConnections).map(
                      ([sourceIdStr, targetIds]) => {
                        ...
                        return targetIds.map((targetId) => {
                          ...
                          const sourceMoodIndex = Object.keys(
                            nodesByMood
                          ).indexOf(sourceNode.mood);
                          const targetMoodIndex = Object.keys(
                            nodesByMood
                          ).indexOf(targetNode.mood);
                          const sourceMoodAngle =
                            ((2 * Math.PI) / Object.keys(nodesByMood).length) *
                            sourceMoodIndex;
                          const targetMoodAngle =
                            ((2 * Math.PI) / Object.keys(nodesByMood).length) *
                            targetMoodIndex;

                          const sourceIndex =
                            nodesByMood[sourceNode.mood].indexOf(sourceNode);
                          const targetIndex =
                            nodesByMood[targetNode.mood].indexOf(targetNode);

                          const sourceAngleOffset =
                            0.2 *
                            (sourceIndex -
                              nodesByMood[sourceNode.mood].length / 2);
                          const targetAngleOffset =
                            0.2 *
                            (targetIndex -
                              nodesByMood[targetNode.mood].length / 2);

                          const sourceX =
                            50 +
                            35 * Math.cos(sourceMoodAngle + sourceAngleOffset);
                          const sourceY =
                            50 +
                            35 * Math.sin(sourceMoodAngle + sourceAngleOffset);
                          const targetX =
                            50 +
                            35 * Math.cos(targetMoodAngle + targetAngleOffset);
                          const targetY =
                            50 +
                            35 * Math.sin(targetMoodAngle + targetAngleOffset);
                  ...
                  {Object.entries(nodesByMood).map(
                    ([mood, moodNodes], moodIndex) => {
                      const angle =
                        ((2 * Math.PI) / Object.keys(nodesByMood).length) *
                        moodIndex;
                  ...
                  {Object.entries(nodesByMood).map(
                    ([mood, moodNodes], moodIndex) => {
                      ...
                      return moodNodes.map((node, nodeIndex) => {
                        const angleOffset =
                          0.2 * (nodeIndex - moodNodes.length / 2);
                        const distance = 40 + (nodeIndex % 2) * 5; // Vary distance slightly
                        const nodeX =
                          50 + distance * Math.cos(moodAngle + angleOffset);
                        const nodeY =
                          50 + distance * Math.sin(moodAngle + angleOffset);
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-2 text-xs text-center">
                    {["image", "audio", "sketch", "thought", "link"].map(
                      (type) => {
