...
const CanvasElement = forwardRef<CanvasElementRef, CanvasElementProps>((props, ref) => {
  const {
    width = 400,
    height = 300,
    color = '#4FC3F7',
  ...
  const endDrawing = () => {
    ...
    if (onDrawEnd && points.length > 1) {
      ...
      // Add to history
      setHistory(prev => [...prev.slice(0, historyIndex + 1), svgContent]);
  ...
  const clearCanvas = () => {
    ...
    `;
    setHistory(prev => [...prev.slice(0, historyIndex + 1), emptySvg]);
    setHistoryIndex(prev => prev + 1);
  ...
  const undoOperation = () => {
    ...
    setHistoryIndex(prev => prev - 1);
    const prevSvg = history[historyIndex - 1];
  ...
  const redoOperation = () => {
    ...
    setHistoryIndex(prev => prev + 1);
    const nextSvg = history[historyIndex + 1];
