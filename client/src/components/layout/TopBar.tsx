...
const TopBar: React.FC = () => {
  ...
  const createTeamMoodMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["/api/projects", variables.projectId, "mood-summary"],
      });
    },
  });
  ...
  const teamAura: TeamAuraData = moodSummary
    ...
    : {
        dominantMood: "energetic" as MoodType,
        alignment: 50,
        description: "Team mood not set",
      };
  ...
  return (
    <header className="glass-panel border-b border-gray-800 px-6 py-3 flex items-center justify-between z-100">
        ...
        {projectId && (
          <div className="flex items-center px-3 py-1.5 rounded-full bg-secondary ml-6">
            <div
              ...
              style={{
                backgroundColor:
                  teamAura.dominantMood === "calm"
                    ? "hsl(var(--calm))"
                    : teamAura.dominantMood === "energetic"
                    ? "hsl(var(--energetic))"
                    : "hsl(var(--focused))",
              }}
            />
            <span className="text-sm">
              {teamAura.dominantMood.charAt(0).toUpperCase() +
                teamAura.dominantMood.slice(1)}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center space-x-5">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex -space-x-1">
              <Button
                ...
                className={`w-5 h-5 p-0 rounded-full bg-[hsl(var(--calm))] hover:ring-2 hover:ring-white ${
                  moodSummary && moodSummary.calm && moodSummary.calm.count > 0
                    ? "ring-2 ring-white"
                    : ""
                }`}
                title="Calm"
              ...
              <Button
                ...
                className={`w-5 h-5 p-0 rounded-full bg-[hsl(var(--energetic))] hover:ring-2 hover:ring-white ${
                  moodSummary &&
                  moodSummary.energetic &&
                  moodSummary.energetic.count > 0
                    ? "ring-2 ring-white"
                    : ""
                }`}
                title="Energetic"
              ...
              <Button
                ...
                className={`w-5 h-5 p-0 rounded-full bg-[hsl(var(--focused))] hover:ring-2 hover:ring-white ${
                  moodSummary &&
                  moodSummary.focused &&
                  moodSummary.focused.count > 0
                    ? "ring-2 ring-white"
                    : ""
                }`}
                title="Focused"
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button
                  variant="ghost"
                  className="w-5 h-5 p-0 rounded-full bg-surface flex items-center justify-center hover:bg-secondary"
                  title="More mood options"
                >
                  <svg
              ...
              <DialogContent className="max-w-md">
                <DialogHeader>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Define the emotional goals for this project. Select multiple
                    moods to create a nuanced emotion profile.
                  </p>
                </DialogHeader>
                <div className="grid grid-cols-3 gap-3 my-4">
                  {["calm", "energetic", "focused", "playful", "serious"].map(
                    (mood) => (
                      <Button
                        key={mood}
                        variant="outline"
                        ...
                        className={`flex flex-col items-center p-3 h-auto gap-2 ${
                          moodSummary &&
                          moodSummary[mood as keyof typeof moodSummary]?.count >
                            0
                            ? "border-primary"
                            : ""
                        }`}
                      >
                        <div
                          className={`w-10 h-10 rounded-full ${
                            moodSummary &&
                            moodSummary[mood as keyof typeof moodSummary]
                              ?.count > 0
                              ? "ring-2 ring-primary"
                              : ""
                          }`}
                  ...
                  <Button
                    ...
                    onClick={() => {
                      toast({
                        title: "Project Mood Updated",
                        description:
                          "The emotional profile for this project has been updated.",
                      });
                    }}
                  >
                    Save Project Mood
                ...
                {notifications.map((notification, index) => (
                  ...
                  >
                    <div className="flex items-start">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center ${notification.color} text-white mr-3 flex-shrink-0`}
                      >
        ...
        <Link href="/profile">
          <div className="w-10 h-10 rounded-full border-2 border-background bg-primary flex items-center justify-center text-white cursor-pointer hover:ring-2 hover:ring-primary-foreground">
            NR
          </div>
        </Link>
      </div>
